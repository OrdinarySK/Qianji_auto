...
public class Ordinary implements IApp{
    private static Ordinary ordinary;
    static long time = 0;

    public static Ordinary getInstance() {
        if(ordinary==null) {
    ...
    public int getAppIcon() {
        return R.drawable.logo_ordinary;
    }
            ...
            public void handleMessage(@NonNull Message msg) {
                ...
                if (msg.what == 0) {
                    ...
                    if (RootUtils.hasRootPermission()) {
                        RootUtils.exec(new String[]{"am start \"" + getOrdinary(billInfo) + "\""});
                    } else {
                        Tool.goUrl(context, getOrdinary(billInfo));
                    }
    ...
    public void asyncDataBefore(Context context, int type) {
        if (AppStatus.isXposed()) {
            Log.i("自动记账同步", "同步开始");
            RootUtils.exec(new String[]{"am force-stop com.sk.ordinary"});
            //杀死其他应用
            //  Tool.stopApp(context,"com.mutangtech.qianji");
            Intent intent = new Intent();
            intent.setClassName("com.sk.ordinary", "com.sk.ordinary.MainActivity");
            intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            intent.putExtra("AutoSignal", type);
            MMKV mmkv = MMKV.defaultMMKV();
            mmkv.encode("AutoSignal", type);
            Log.i("自动记账同步", "正在前往Ordinary记账");
    ...
    public String getAsyncDesc(Context context) {
        if (AppStatus.xposedActive(context)) {
            return context.getResources().getString(R.string.ordinary_async_desc);
        }
        return context.getResources().getString(R.string.ordinary_async_no_support);
    }
    ...
    public String getOrdinary(BillInfo billInfo) {
        String url = "ordinary://sk/addBill?type="+ billInfo.getType(true) + "&money=" + billInfo.getMoney();
        MMKV mmkv = MMKV.defaultMMKV();
